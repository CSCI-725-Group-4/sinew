...
As mentioned in the original paper, Query 4 performs a query for array elements within the database. The query written to perform this on out PostgreSQL benchmark looked like the following:
\begin{verbatim} 
SELECT 
    id,
    jsonb_array_elements_text(data -> 'nested_arr') 
    AS nested_arr_elements
FROM json_data;
\end{verbatim}
